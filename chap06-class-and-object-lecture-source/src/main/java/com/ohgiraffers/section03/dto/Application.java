...
public class Application {
    public static void main(String[] args) {
        ...
        * 어떤 것을 쓸줄 모르니 미리 다 준비해두는 종합선물 세트 같은 개념
        * private 필드와 필드값을 수정하는 설정자(setter) , 필드에 접근하는 접근자 (getter) 구성
        ...
        * 필드명을 그대로 사용한 설정자와 접근자로 인해 캡슐화의 효과가 없다. 유지 보수성 저하
        * 하지만 데이터를 주로 다루는 객체의 경우 행위를 추상화하지 않고
        * 미리 모든 필드에 접근 가능성을 염두해 두고 작성해두는 관례로 인해 현재도 많이 사용하고 있다.
        *
        * */
    }
}
