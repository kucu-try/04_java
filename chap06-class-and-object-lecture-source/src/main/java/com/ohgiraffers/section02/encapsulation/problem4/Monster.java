...
public class Monster {
    ...
    /*
    *  접근 제한자
    * 클래스 혹은 클래스의 멤버에 참조 연산자로 접근할 수 있는 범위르 제한하기 위한 키워드이다
    * 1. public : 모든 패키지에 접근 허용
    * 2. protected : 동일 패키지에 접근 허용 단, 상속 관계에 있는 경우 다른 패키지에도 접근 가능
    * 3. default : 동일 패키지에서만 접근 허용( 기본 값으로 작성하지 않으면 default 접근 제한을 갖는다.
    * 4. private : 해당 클래스 내부에서만 접근을 허용
    *
    * 위의 네가지 접근 제한자는 클래스의 멤버(필드, 메소드)에 모두 사용이 가능하다.
    ...
    * */
    ...
    public void setHp(int hp){
        if(hp >0){
            ...
            this.hp =0;
            System.out.println("체력이 0보다 적으면 죽어! 이러다 다 죽어~");
        }
    }
