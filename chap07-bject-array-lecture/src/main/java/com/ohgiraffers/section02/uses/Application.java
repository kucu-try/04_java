...
public class Application {
    public static void main(String[] args){
        ...
        * 2, 회원 정보는 최대 10명 까지 저장할 수 있다.
        * */

        //사용자의 입력을 받을수 있도록 객체 생성
        //반복문 내부에서 생성시 반복문 실행마다 새로운 인스턴스 생성해야하기 떄문에
        ...
        while (true){
            ...
            switch (no){ // 입력값이 아래와 같을 경우 해당되는 서비스를 실행해줌
                // 사용자 입력 값이 == 1 이면 memberService클래스에 있는 signUpMembers() 메서드를 호출함
                //호출시 stack 영역에서 동작이 된다. 참고 ((no==1) - 1)
                case 1 : memberManager.signUpMembers(); break;
                //사용자 입력 값이 ==2면 memberService클래스에 있는 showAllMembers() 메서드를 호출한다. ((no==2) - 1)
                case 2 : memberManager.showAllMembers(); break;

                case 9 :
                    System.out.println("프로그램을 종료합니다"); return;
                default:
                    System.out.println("잘못된 번호를 입력함"); break;
            }
        }
    }
}
